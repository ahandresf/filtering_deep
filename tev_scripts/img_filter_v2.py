...
def main():
    ...
    output_file=OUTPUT_DIR+input_file.split('.npy')[0]+'_fil2.npy'
    ...
    for i in range(num_img):
    	img_2=img[i,:,:,:]
        fil_sig, fil_sig_n=filter_image(img_2) #filter the image
	L.append(fil_sig)
    output=np.stack(L) #stack of all images with different filters
    print('output shape',output.shape)
    np.save(output_file,output) #store the image
